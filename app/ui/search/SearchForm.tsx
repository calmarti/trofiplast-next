...
export default function SearchForm({isMounted,isLoading,fieldOptions, handleChange, selection }){
    ...
    const handleSubmit = async() => {
      // ev.preventDefault();
      const results = await serverAction(selection);
      console.log(results);
    }
    ...
    <div className="max-w-full flex flex-col justify-center items-center gap-6 sm:flex-row sm:gap-12 my-6">
      ...
      <div className="flex flex-col">
        ...
        <button type="submit" 
        className="w-24 focus:outline-none text-white bg-green-700 hover:bg-green-600 focus:ring-green-600 active:bg-green-500 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mt-10 mb-36">Submit</button>
  </form>);
}
